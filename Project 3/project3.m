...
while(1)
    ...
    for i = 1:length(WQReceiveTimes)
        ...
        if WQReceiveTimes(i) == time && time ~= 0
            ...
            %if WQ(i) == receiverSequence || WQ(i) == receiverSequence - 1
                if WQ(i) == receiverSequence
                    % receiver sends ACK#receiverSequence+1
                    receiverSequence = receiverSequence + 1;
                end
                if ackGood(time+1) == 1
                    AQStatus(i) = 1;
                    AQReceiveTimes(i) = time + ackDelay + propogationDelay;
                else
                    AQStatus(i) = 1;
                    % ACK is lost
                    AQReceiveTimes(i) = 0;
                end
                receiverOutput = getTimeString(time) + "receiver got frame " + WQ(i) + ", transmitting ACK" + receiverSequence + ", " + getTransmissionString(ackGood(time+1));
                disp(receiverOutput);
                break;
            %end
        end
