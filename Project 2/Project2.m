...
function correct_bytes = ecc_hamming(error_bytes) 
    ...
    data = error_bytes(1:4);
    parity = error_bytes(5:7);
    formed_parity = [xor(data(1),xor(data(2),data(3))), xor(data(2),xor(data(3),data(4))), xor(data(1),xor(data(2),data(4)))];
    errindx = xor(parity, formed_parity);
    num = bit2int(errindx', 3, true);
    if(num ~= 0)
        switch num
            case 1
            case 2
            case 3
                % 4th bit is wrong
                error_bytes(4) = xor(error_bytes(4), 1);
            case 4
            case 5
                % 1st bit is wrong
                error_bytes(1) = xor(error_bytes(1), 1);
            case 6
                % 3rd bit is wrong
                error_bytes(3) = xor(error_bytes(3), 1);
            case 7
                % 2nd bit is wrong
                error_bytes(2) = xor(error_bytes(2), 1);
        end
    end
